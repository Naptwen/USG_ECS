#################################################
##  GNU Affero 3.0 License Useop Gim 2023-04-23 #
#################################################
...
#define _usg_any_cast(_TYPE, KEY) std::any_cast<_TYPE>(bucket_list.at(KEY).at(typeid(std::remove_reference_t<_TYPE>)))
...
class HASH_TABLE{
    ...
    void _Filter(const std::string_view key, size_t& n, _Tuple& tup){
        if(bucket_list.at(key).count(typeid(_T1)) && ++n){
