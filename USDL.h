...
namespace usdl
{
	...
	using AnimationList = std::vector<Vec2i>;
	...
	public:
		...
		void SetAnim(bool loop, int index_row = 0)
		{
			SDL_assert(split.x != 0 && split.y != 0);
			_loop = loop;
		...
		void GetPostion(int& px, int& py)
		{
			px = DrawSize.x;
			py = DrawSize.y;
		}
		void GetDrawSize(int& w, int& h)
		{
			w = DrawSize.w;
			h = DrawSize.h;
		}
		void GetOriginSize(int& w, int& h)
		{
			w = OriginSize.w;
			h = OriginSize.h;
		}
	...
	struct Text
	{
		...
		void Font(const char* font_path, unsigned int font_size, SDL_Color color)
		{
		...
		{
			_text = text;
			_textRect.w = static_cast<int>(std::strlen(_text) * ((_font_size) >> 1));
			_textRect.h = _font_size;
		}
	...
	struct Music
	{
		...
		void Init(const char* music_path, TYPE type)
		{
			_type = type;
			if (_type == TYPE::music)
